...
public class PTagValueDAO {
    public void addTagValue(TagData tagData) throws SQLException {
        String sql = "INSERT INTO ptag_value(tag_id, tag_time, tag_value) VALUES(?, ?, ?) on conflict (tag_id, tag_time) do update set tag_value = ?, updated_at = CURRENT_TIMESTAMP";
    ...
    public TagData getTagValue(int tagId, Timestamp tagTime) throws SQLException {
        String sql = "SELECT * FROM ptag_value WHERE tag_id = ? and tag_time = ?";
        try (Connection conn = DatabaseConnection.getConnection();
